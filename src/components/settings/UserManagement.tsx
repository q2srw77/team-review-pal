...
export default function UserManagement() {
  ...
  const [inviteForm, setInviteForm] = useState({
    full_name: "", email: "", password: "", roles: [] as AppRole[],
  });
  ...
  const handleEditUser = async () => {
    ...
    if (!editForm.full_name.trim()) { toast({ title: "Name is required", variant: "destructive" }); return; }
    if (!editForm.email.trim()) { toast({ title: "Email is required", variant: "destructive" }); return; }
    if (editForm.password && editForm.password.length < 6) {
      toast({ title: "Password must be at least 6 characters", variant: "destructive" }); return;
    }
    setSaving(true);
    const body: Record<string, string> = {
      action: "update_user", user_id: editTarget.user_id,
  ...
  const RoleCheckboxGroup = ({ selected, onChange }: { selected: AppRole[]; onChange: (roles: AppRole[]) => void }) => (
    <div className="space-y-3">
      {ALL_ROLES.map((r) => (
        <label key={r.value} className="flex items-center gap-2 cursor-pointer">
          <Checkbox checked={selected.includes(r.value)} onCheckedChange={() => onChange(toggleRole(selected, r.value))} />
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-2">
              ...
              <Input value={inviteForm.full_name} onChange={(e) => setInviteForm((f) => ({ ...f, full_name: e.target.value }))} placeholder="Jane Doe" />
            </div>
            <div className="space-y-2">
              <Label>Email</Label>
              <Input type="email" value={inviteForm.email} onChange={(e) => setInviteForm((f) => ({ ...f, email: e.target.value }))} placeholder="jane@example.com" />
            </div>
            <div className="space-y-2">
              <Label>Password</Label>
              <Input type="password" value={inviteForm.password} onChange={(e) => setInviteForm((f) => ({ ...f, password: e.target.value }))} placeholder="Min 6 characters" />
            </div>
            <div className="space-y-2">
              <Label>Roles</Label>
              <RoleCheckboxGroup selected={inviteForm.roles} onChange={(roles) => setInviteForm((f) => ({ ...f, roles }))} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setInviteOpen(false)}>Cancel</Button>
            <Button onClick={handleInvite} disabled={inviting}>{inviting ? "Inviting..." : "Invite"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
